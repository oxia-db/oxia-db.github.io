...
key) to determine shard routing. The record is still stored and retrieved by its primary key — only
...
<Tabs items={['Go', 'Java', 'Python', 'Node.js', 'CLI']}>
    ...
    <Tabs.Tab>
        ```ts
        // Store related records on the same shard
        await client.put('/users/123/profile', 'profile-data', {
          partitionKey: '/users/123',
        });
        await client.put('/users/123/settings', 'settings-data', {
          partitionKey: '/users/123',
        });

        // List all records for user 123 (works because they share a partition key)
        const keys = await client.list('/users/123/', '/users/123//', {
          partitionKey: '/users/123',
        });

        // Delete all records for user 123
        await client.deleteRange('/users/123/', '/users/123//', {
          partitionKey: '/users/123',
        });
        ```
    </Tabs.Tab>
