...
index name and a secondary key. The secondary key does not need to be unique — multiple records can
...
<Tabs items={['Go', 'Java', 'Python', 'Node.js', 'CLI']}>
    ...
    <Tabs.Tab>
        ...
        await client.put('/users/123',
          ...
          { secondaryIndexes: { 'by-email': 'alice@example.com' } },
        );
        ```
    </Tabs.Tab>
    <Tabs.Tab>
...
<Tabs items={['Go', 'Java', 'Python', 'Node.js', 'CLI']}>
    ...
    <Tabs.Tab>
        ```ts
        // Lookup by secondary index
        const { key, value, version } = await client.get('alice@example.com', {
          useIndex: 'by-email',
        });

        // List all keys in a secondary index range
        const keys = await client.list('a', 'b', { useIndex: 'by-email' });

        // Range scan using secondary index
        const decoder = new TextDecoder();
        for await (const r of client.rangeScan('a', 'b', { useIndex: 'by-email' })) {
          console.log(`primary key: ${r.key}, value: ${decoder.decode(r.value)}`);
        }
        ```
    </Tabs.Tab>
    <Tabs.Tab>
