...
the cluster, which instead falls within the scope of the [coordinator section](replication-coordinator.mdx) of the
protocol.
...
then attempt to forward the entry to all of its followers who will in turn accept and persist it to their WALs,
...
In the example below we see that `e4` has been successfully written to the logs of a quorum of followers, and that `e5`
...
Note over S,WAL: Up to this point any Get(x) would return 3
