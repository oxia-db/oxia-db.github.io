...
Building on the [design goals](./design-goals.mdx), the logical architecture is organized around three concepts:
...
![Sharding](/architecture/logical-architecture-2.png)
...
Oxia supports sharding of keys, which means that the keys used to access data are distributed across various shards.
...
![Leader-Based architecture](/architecture/logical-architecture-3.png)

The leader-based architecture is designed to optimize the management of data and tasks in a distributed system.
...
In Oxia, the leader-based design ensures that one node is designated as the leader at any given time.
