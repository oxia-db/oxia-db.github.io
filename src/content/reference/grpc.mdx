...
server. Application developers using the official [Go](../clients/go.mdx),
[Java](../clients/java.mdx), or [Python](../clients/python.mdx) SDKs do not need to read
this — the SDKs wrap everything documented below.
...
| `OxiaAdmin` | Data server and namespace management, shard split | Coordinator | Operators, admin CLI |
...
| RPC | Request → Response | Purpose |
|---|---|---|
| `ListDataServers` | `ListDataServersRequest` → `ListDataServersResponse` | Enumerate configured data servers. |
| `GetDataServer` | `GetDataServerRequest` → `GetDataServerResponse` | Read a configured data server. |
...
| `GetNamespace` | `GetNamespaceRequest` → `GetNamespaceResponse` | Read a namespace by name. |
...
The admin service is implemented only by the coordinator. The request and response
messages are listed in the draft [Admin API](./admin-api.mdx) reference.
