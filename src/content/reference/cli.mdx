...
| `--auth-token-file` | — | Path to a bearer token file for authenticated requests. |
| `-o, --output` | — | Output format: `json`, `yaml`, `name`, or `table`. |

### `oxia admin namespace get`

Get one namespace, or list all namespaces when no namespace name is provided.
...
| Flag | Default | Purpose |
|---|---|---|
| `--initial-shards` | `0` | Initial shard count. Required. |
| `--replication-factor` | `0` | Replication factor. Required. |
| `--notifications` | `true` | Enable notifications for the namespace. |
| `--key-sorting` | `hierarchical` | Key sort mode: `hierarchical` or `natural`. |

### `oxia admin namespace patch`

Patch mutable namespace settings.

```shell
oxia admin namespace patch my-namespace --replication-factor 5
oxia admin namespace patch my-namespace --notifications=false
```

| Flag | Default | Purpose |
|---|---|---|
| `--replication-factor` | `0` | New replication factor. |
| `--notifications` | `true` | Enable or disable notifications. |

### `oxia admin namespace delete`
...
Get one data server, or list all data servers when no data server name is provided.
...
### `oxia admin dataserver create`

Create a data server entry.
...
| Flag | Default | Purpose |
|---|---|---|
| `--public` | — | Public address for client traffic. Required. |
| `--internal` | — | Internal address for coordinator and peer traffic. Required. |
| `--label` | — | Label in `key=value` form. Can be repeated. |

### `oxia admin dataserver patch`

Patch a data server entry. At least one field must be provided.
...
| `--internal` | — | New internal address. |
| `--label` | — | Replacement labels in `key=value` form. Can be repeated. |

### `oxia admin dataserver delete`

Delete a data server entry.

```shell
oxia admin dataserver delete server-1
```

### `oxia admin shard split`
