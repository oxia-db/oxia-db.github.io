...
- **Records.** Every record is a key (string), a value (bytes), and a per-key monotonic
  `VersionId` used for optimistic concurrency. Keys are flat; path-shaped keys like `/users/42`
  are the recommended style. See [key sorting](./features/oxia-key-sorting.mdx) for
  hierarchical vs natural ordering.
